...
def update_live_data():
    ...
    st.session_state.drone_data = {
        'drone_1': {
            ...
            'detections': [
                {'type': 'Plastic', 'time': '14:39:32', 'severity': 'High'},
                {'type': 'Oil', 'time': '12:52:50', 'severity': 'Medium'},
                {'type': 'Plastic', 'time': '16:14:20', 'severity': 'High'},
                {'type': 'Oil', 'time': '10:30:05', 'severity': 'Low'}
            ]
        },
        'drone_2': {
            'status': 'Active',
    ...
    for drone_name, data in st.session_state.drone_data.items():
        ...
        if drone_name == 'drone_1' and data['detections']:
            ...
            for detection in data['detections'][:3]:
                ...
                st.markdown(f"• **{detection['type']}** - {detection['time']} - <span style='color: {severity_color[detection['severity']]}; font-weight: bold;'>{detection['severity']}</span>", unsafe_allow_html=True)
...
# SMART BUOY PAGE
    ...
    positions = [
        (0, 1, 0.5, 1),      # Top row, column 1
        (0.25, 1, 0.5, 1),   # Top row, column 2
        (0.5, 1, 0.5, 1),    # Top row, column 3
        (0.75, 1, 0.5, 1),   # Top row, column 4
        (0, 1, 0, 0.5),      # Bottom row, column 1
        (0.25, 1, 0, 0.5),   # Bottom row, column 2
        (0.5, 1, 0, 0.5),    # Bottom row, column 3
